...
exports.init = function() {
    // TODO: Setup GPIO pin for input analog switch
...
exports.updateLights = function(options, callback) {
    ...
    if (options.intensity && options.pin) {
        // TODO: Manual controlling of the lights.
        callback();
    }

    if (now.getDay() > internals.update.lastDay || (now.getDay() === 0 && internals.update.lastDay === 6)) {
        ...
        if (config.controllerMode === "auto") { // Get the ephemeris times, only if in auto mode.
            ...
            internals.update.sunTimes = ephemeris.Sun.getTimes(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), config.latitude, config.longitude);
            internals.update.moonTimes = ephemeris.Moon.getTimes(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), now.getTimezoneOffset()/60*-1, config.latitude, config.longitude);
        }
    }

    switch  (config.controllerMode) {
        case "auto":
            if (now > ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][2]) || now <= ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][1])) {
                internals.update.dayStatus = "Night";
            } else if (now > ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][2]) && now <= ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][2])) {
                internals.update.dayStatus = "Sunset";
            } else if (now > ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][1]) && now <= ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][2])) {
                internals.update.dayStatus = "Day";
            } else if (now > ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][1]) && now <= ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][1])) {
                internals.update.dayStatus = "Sunrise";
            } else {
                internals.update.dayStatus = "Error";
                internals.update.dayError = {
                    sunriseStart: ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][1]),
                    sunsetEnd: ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][2]),
                    sunriseEnd: ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][1]),
                    sunsetStart: ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[0][2])
                };
            }
            break;
    ...
    if (isNight === 1) {
        if (config.controllerMode === 'auto' && config.simulateMoon) {
            ...
            if (isUp) {
                ...
                var fracHours = now.getUTCHours() + (now.getUTCMinutes()/60) + (now.getUTCSeconds()/(60*60));
                phase = ephemeris.Moon.getFraction(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), fracHours);
            } else {
                // Moon is set
                phase = 0;
            }
        ...
        for (var i = 0; i < numChannels; i++) {
            if (isSunset === 0) { // Sunrise
                if (config.controllerMode === 'auto') {
                    ...
                    internals.pwm[i] = Math.round(config.pwmSettings[0][i] * Math.sin((Math.PI / 2) * (Math.max((now.getTime() - ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes[1][1]).getTime() - (config.pwmSettings[2][i]*1000)), 0) / rampTime)));
                } else {
                ...
                if (config.controllerMode === 'auto') {
                    ...
                    internals.pwm[i] = Math.round(config.pwmSettings[0][i] * Math.cos((Math.PI / 2) * (Math.min((now.getTime() - ephemeris.fromUTCHours(now.getUTCFullYear(), now.getUTCMonth()+1, now.getUTCDate(), internals.update.sunTimes.sunsetStart).getTime() + (config.pwmSettings[2][i]*1000)), rampTime) / rampTime)));
                } else {
